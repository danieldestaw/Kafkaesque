...
func (s *Server) login(w http.ResponseWriter, r *http.Request) {
	if s.cfg.OIDCEnabled && !s.cfg.LocalLoginEnabled {
		writeError(w, http.StatusForbidden, "LOCAL_LOGIN_DISABLED", "Local login is disabled; use SSO")
		return
	}
...
func (s *Server) fetchMessages(w http.ResponseWriter, r *http.Request) {
	...
	topic := chi.URLParam(r, "topic")
	partitionQuery := strings.TrimSpace(r.URL.Query().Get("partition"))
	offsetQuery := strings.TrimSpace(r.URL.Query().Get("offset"))
	limit, _ := strconv.Atoi(r.URL.Query().Get("limit"))

	if partitionQuery == "" && offsetQuery == "" {
		result, err := s.kafka.TailMessages(r.Context(), c, topic, limit, r.URL.Query().Get("cursor"))
		if err != nil {
			writeError(w, http.StatusBadGateway, "KAFKA_ERROR", err.Error())
			return
		}
	...
	partition, _ := strconv.ParseInt(partitionQuery, 10, 32)
	var offset int64 = -1
	if offsetQuery != "" {
