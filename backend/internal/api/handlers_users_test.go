...
func TestChangeOwnPassword(t *testing.T) {
	...
	srv.Router().ServeHTTP(rec, authRequest(http.MethodPost, "/api/v1/users", admin, map[string]any{
		"email": "pwtest@streamforge.local", "display_name": "PW Test",
	...
	token := loginToken(t, srv, "pwtest@streamforge.local", "oldpass1")
	...
	// New password works
	newToken := loginToken(t, srv, "pwtest@streamforge.local", "newpass1")
	rec = httptest.NewRecorder()
