...
export function Drawer({ open, onClose, title, children, width = 'lg' }: Props) {
  ...
  const widthClass =
    width === 'xl' ? 'sm:max-w-2xl' : width === 'md' ? 'sm:max-w-md' : 'sm:max-w-lg'
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        ...
        aria-modal
        className={cn(
          'relative flex h-full w-full max-w-full flex-col border-l border-sf-border bg-sf-panel shadow-2xl animate-slide-left',
          widthClass,
        )}
      >
